...
export default function RegisterPage() {
  ...
  const generateVerificationCode = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/habbo/user/${encodeURIComponent(habboNick.trim())}`,
      );
      if (!res.ok) {
        setVerificationError(
          `El personaje '${habboNick}' no fue encontrado en Habbo.es. Asegúrate de escribirlo correctamente.`,
        );
      ...
      setVerificationError(
        "Error de conexión al buscar el personaje. Por favor, inténtalo de nuevo.",
      );
    } finally {
      setCheckingNickExists(false);
    }
  ...
  const handleVerifyMotto = async () => {
    ...
    try {
      const res = await fetch(
        `/api/habbo/user/${encodeURIComponent(habboNick)}`,
      );
      if (!res.ok) {
        setVerificationError(
          "No se pudo obtener tu perfil de Habbo. Inténtalo más tarde.",
        );
      ...
      setVerificationError(
        "Error de conexión al conectar con la API de Habbo. Por favor, vuelve a intentarlo.",
      );
    } finally {
      setIsVerifying(false);
    }
          ...
          <Card className="bg-card border-border glow-purple flex-1 overflow-hidden relative glass">
            ...
            <CardContent className="pt-4">
              ...
              {error && (
                <Alert
                  variant="destructive"
                  className="py-2 mb-4 bg-destructive/10 border-destructive/20 text-destructive text-xs"
                >
              ...
              {!verified && (
                <div className="space-y-4">
                  {verificationError && (
                    <Alert
                      variant="destructive"
                      className="py-2.5 bg-destructive/10 border-destructive/20 text-destructive text-xs"
                    >
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && generateVerificationCode()
                            }
                            data-testid="input-verify-habbonick"
                          />
                        </div>
                        <p className="text-[10px] text-muted-foreground mt-1.5 leading-relaxed">
                          Ingresa tu nombre de Habbo. Generaremos un código
                          exclusivo que deberás poner en tu misión para
                          verificar tu identidad y evitar que otros usen tu
                          nombre.
                        </p>
                      </div>
                    ...
                    <div className="space-y-4 animate-in fade-in duration-300">
                      <div className="bg-secondary/40 border border-border rounded-xl p-3.5 space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] bg-primary/20 text-primary border border-primary/30 py-0.5 px-2 rounded-full font-semibold">
                            De un solo uso
                          </span>
                        </div>
                      ...
                      <div className="space-y-2 text-xs text-muted-foreground leading-relaxed">
                        ...
                        <ol className="list-decimal list-inside space-y-1 bg-secondary/20 p-2.5 rounded-lg border border-border/30">
                          <li>
                            Inicia sesión con tu personaje{" "}
                          ...
                          <li>
                            Cambia tu **misión** (motto) por exactamente:{" "}
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Cambiar
                          Nick
                        </Button>
                        <Button
