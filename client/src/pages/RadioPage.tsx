  ...
  const sendRequestMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Tu canción ha sido solicitada al DJ.",
      });
  ...
  const sendSaludoMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Tu dedicatoria ha sido enviada al DJ.",
      });
      ...
      <div className="site-panel-strong p-5 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-2 max-w-2xl">
            Sintoniza nuestra música 24/7, pide tus temas favoritos, chatea en
            tiempo real con la comunidad y envíale saludos al locutor de turno.
          </p>
        </div>
          ...
          <Card className="border border-border bg-card/60 backdrop-blur-md overflow-hidden relative shadow-lg flex-1 flex flex-col">
            ...
            <CardContent className="p-6 flex-1 flex flex-col items-center justify-center space-y-6">
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        animation: isPlaying
                          ? `bounceEqualizer ${animDuration}s ease-in-out infinite alternate`
                          : "none",
              ...
              <div className="text-center space-y-1 max-w-sm">
                <h3
                  className="text-sm font-black text-white truncate px-2"
                  title={songTitle}
                >
            ...
            <Card className="border border-border bg-card/60 backdrop-blur-md flex flex-col overflow-hidden h-[480px] md:h-auto">
              ...
              <CardContent className="p-0 flex-1 flex flex-col overflow-hidden">
                  ...
                  {chatMessages.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-center p-4">
                      <p className="text-xs text-muted-foreground">
                        No hay mensajes en el chat de la radio. ¡Sé el primero!
                      </p>
                    </div>
                  ) : (
                    chatMessages
                      ...
                      .map((msg, i) => (
                        <div
                          key={msg.id || i}
                          className="text-xs bg-zinc-950/45 p-2.5 rounded-lg border border-border/30 space-y-1"
                        >
                ...
                <form
                  ...
                  className="p-3 border-t border-border bg-black/15 flex gap-2"
                >
                  {user ? (
                    <>
                      <Input
              ...
              >
                <CardHeader className="py-2 px-3 border-b border-border bg-secondary/15">
                  <TabsList className="grid grid-cols-2 h-8 bg-zinc-950/60 p-0.5 border border-border/40">
                    <TabsTrigger
                      value="pedir"
                      className="text-[10px] uppercase font-bold py-1 flex items-center justify-center gap-1.5"
                    >
                ...
                <CardContent className="p-4 flex-1 flex flex-col justify-between">
                  <TabsContent
                    value="pedir"
                    className="mt-0 space-y-4 flex-1 flex flex-col justify-between"
                  >
                    <div className="space-y-3">
                      <div>
                        <Label className="text-[9.5px] uppercase font-black tracking-wider text-slate-400">
                          Título de Canción
                        </Label>
                        <Input
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setSaludoForm({ details: e.target.value })
                          }
                      ...
                      <div className="bg-zinc-900 border border-border/40 rounded-xl p-3 text-[10px] text-muted-foreground leading-relaxed">
                        <p className="font-bold text-white mb-0.5 flex items-center gap-1">
                          <Sparkles className="w-3 h-3 text-primary" /> Consejos
                          del Locutor
                        </p>
                        Los locutores leen los saludos al aire durante las
                        transmisiones en vivo. Escribe claro y con respeto.
                      </div>
                    </div>
                    <Button
                      onClick={handleSendSaludo}
                      disabled={
                        sendSaludoMutation.isPending ||
                        !saludoForm.details.trim()
                      }
                      className="w-full h-8 text-xs bg-primary text-black font-black hover:bg-primary/95 mt-4"
                    >
                      Enviar Saludo
            ...
            <CardContent className="p-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {nowPlaying?.song_history?.slice(0, 4).map((h, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2.5 bg-zinc-950/40 border border-border/40 p-2 rounded-lg"
                  >
                    <img
                      src={proxyImage(
                        h.song?.art ||
