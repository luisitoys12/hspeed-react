...
export default function SongHistoryPage() {
  ...
  const requestMutation = useMutation({
    mutationFn: async (songStr: string) => {
      if (!user)
        throw new Error("Debes iniciar sesión para pedir una canción.");
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {topSongs.slice(0, 3).map((song, index) => {
              ...
              return (
                <Card
                  key={song.id || index}
                  className={`relative overflow-hidden border ${bgTiers[index] || "border-border"}`}
                >
                  <CardContent className="p-6 flex items-center gap-4">
                    <div className="relative flex-shrink-0">
                      ...
                      />
                      <span
                        className={`absolute -top-2 -left-2 w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${badgeColors[index] || "bg-card text-white"}`}
                      >
                        {index + 1}
                      </span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <h2 className="text-2xl font-bold uppercase tracking-wide text-white mb-6 font-cabinet flex items-center gap-2">
            <i className="fa-solid fa-compact-disc text-primary spin-slow"></i>{" "}
            Timeline de Emisión
          </h2>
          ...
          {isLoading ? (
            <div className="space-y-4">
              {[1, 2, 3].map((n) => (
                <div
                  key={n}
                  className="h-24 bg-card/40 rounded-xl border border-border animate-pulse"
                />
              ))}
            </div>
          ) : history.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-border rounded-xl bg-card/20">
              ...
              <p className="text-muted-foreground text-sm">
                No se ha registrado música recientemente.
              </p>
            </div>
          ) : (
              ...
              {history.map((song, index) => {
                ...
                return (
                  <div key={song.id} className="relative pl-14 group">
                    ...
                    <div
                      className={`absolute left-6 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4.5 h-4.5 rounded-full border-2 bg-background flex items-center justify-center transition-all duration-300 z-10 ${
                        isLiveNow
                          ? "border-primary animate-pulse w-5 h-5 shadow-[0_0_10px_rgba(245,166,35,0.6)]"
                          : "border-border group-hover:border-primary/60"
                      }`}
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${isLiveNow ? "bg-primary" : "bg-muted-foreground"}`}
                      />
                    </div>
                    ...
                    <Card
                      className={`border transition-all duration-300 ${
                        isLiveNow
                          ? "border-primary/50 bg-gradient-to-r from-primary/5 to-card"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-[10px] text-muted-foreground">
                            <span className="flex items-center gap-1">
                              ...
                              {formatDistanceToNow(new Date(song.playedAt), {
                                addSuffix: true,
                                locale: es,
                              })}
                            </span>
          ...
          <Card className="border border-border bg-card/60 backdrop-blur sticky top-24">
            <div className="p-6">
              ...
              <div className="text-xs text-muted-foreground space-y-4 leading-relaxed">
                <p>
                  Cada vez que un DJ reproduce un tema en nuestra emisora, el
                  historial se actualiza en tiempo real.
                </p>
                <p>
                  Si tienes tu cuenta de HabboSpeed conectada, puedes hacer clic
                  en <strong className="text-white">"Pedir otra vez"</strong>{" "}
                  para enviar instantáneamente el nombre del tema al sistema de
                  peticiones del DJ.
                </p>
                <p className="border-t border-border/60 pt-4">
                  El DJ en vivo podrá ver tu solicitud y programar la canción en
                  los próximos minutos si encaja con su show actual. ¡Mantente
                  atento a la radio!
                </p>
                {!user && (
                  <div className="bg-primary/5 border border-primary/20 p-3 rounded-lg text-center mt-6">
                    ...
                    <p className="mb-3 text-[11px]">
                      Necesitas ingresar para solicitar canciones directamente.
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full text-xs font-bold bg-primary text-black border-none hover:bg-primary/90"
