...
export default function FeriaPage() {
  ...
  return (
    <div className="min-h-screen bg-[#edf2f7] dark:bg-[#070b14] py-6 px-3 sm:px-4 lg:px-6 font-sans">
      <div className="max-w-[1500px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-5">
        
        {/* ================= COLUMNA IZQUIERDA: SIDEBAR DE PÍLDORAS ================= */}
        <aside className="lg:col-span-2 hidden lg:block space-y-2">
          <div className="bg-white dark:bg-[#0e1626] border border-slate-200/80 dark:border-white/10 rounded-2xl p-3 shadow-sm sticky top-18">
            <div className="text-[10px] font-black uppercase tracking-wider text-slate-400 px-3 py-1.5 mb-1">
              Navegación
            </div>
            <nav className="space-y-1">
              {sidebarNavItems.map((item, idx) => (
                <Link
                  key={idx}
                  href={item.href}
                  className="flex items-center gap-3 px-3 py-2 text-xs font-bold rounded-xl text-slate-700 dark:text-slate-300 hover:text-cyan-500 hover:bg-slate-100 dark:hover:bg-white/5 transition-all cursor-pointer group"
                >
                  <i className={`${item.icon} w-4 text-center text-slate-400 group-hover:text-cyan-500 transition-colors`}></i>
